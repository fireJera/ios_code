...
+ (instancetype)shareCache {
    static VideoCache *cache;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        NSString *cachePath = [NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) firstObject];
        cachePath = [cachePath stringByAppendingPathComponent:@"com.DandJ.Video"];
        VideoDiskCache *diskCache = [[VideoDiskCache alloc] initWithPath:cachePath];
        cache = [[VideoCache alloc] initWithDiskCache:diskCache];
    });
    return cache;
}

#pragma mark - pubulic
- (instancetype)initWithDiskCache:(VideoDiskCache *)diskCache {
    self = [super init];
    if(!self) return nil;
    self.diskCache = diskCache;
    _lock = dispatch_semaphore_create(1);
    _urlFileNameCache = [NSMutableDictionary dictionary];
    _fileQueue = dispatch_queue_create("com.DandJ.Video.cache", DISPATCH_QUEUE_SERIAL);
    return self;
}
