...
+ (void)createGIFfromURL:(NSURL*)videoURL withFrameCount:(int)frameCount intervalTime:(float)intervalTime delayTime:(float)delayTime completion:(void(^)(NSURL *GifURL))completionBlock {
    ...
    // How far along the video track we want to move, in seconds.
    float increment = intervalTime;//(float)videoLength/frameCount;
    ...
    for (int currentFrame = frameCount - 1; currentFrame > 1; --currentFrame) {
        float seconds = (float)increment * currentFrame;
        CMTime time = CMTimeMakeWithSeconds(seconds, [timeInterval intValue]);
        [times addObject:[NSValue valueWithCMTime:time]];
    }
    [timePoints addObjectsFromArray:times];
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        gifURL = [self createGIFforTimePoints:timePoints fromURL:videoURL fileProperties:fileProperties frameProperties:frameProperties frameCount:frameCount * 2 - 2 gifSize:GIFSizeMedium];
...
//        imageRef = (__bridge_retained CGImageRef)tempImage;
