...
+ (UIImage *)hx_imageNamed:(NSString *)imageName {
    ...
    NSString * path = [selfBundle pathForResource:@"HXWeiboPhotoPicker" ofType:@"bundle"];
...
//    if (model.subType == WALMEPhotoModelMediaSubTypePhoto) {
...
//        };
        ...
        [model.asset requestContentEditingInputWithOptions:options completionHandler:^(PHContentEditingInput * _Nullable contentEditingInput, NSDictionary * _Nonnull info) {
            BOOL error = [[info objectForKey:PHContentEditingInputErrorKey] boolValue];
...
+ (NSString *)transFormPhotoTitle:(NSString *)englishName {
    ...
    if ([englishName isEqualToString:@"Bursts"]) {
        ...
        photoName = @"全景照片";
    } else if([englishName isEqualToString:@"Recents"]){
           photoName = @"最近项目";
    } else {
...
+ (NSString *)maximumOfJudgment:(HXPhotoModel *)model manager:(WALMEAlbumListViewmodel *)manager {
    ...
    if (manager.selectionType == WALMEAlbumListSelectionTypeAll) {
        if (model.type == WALMEPhotoModelMediaTypePhoto ||
            model.type == WALMEPhotoModelMediaTypeGif ||
//            model.type == WALMEPhotoModelMediaTypeCameraPhoto ||
            model.type == WALMEPhotoModelMediaTypeLive) {
//            if (manager.maxVideoNum > 0) {
...
////                }
//            }
            if (manager.selectedPhotos.count == manager.maxPhotoNum) {
                // 已经达到图片最大选择数
                return [NSString stringWithFormat:[NSBundle hx_localizedStringForKey:@"最多只能选择%ld张图片"],manager.maxPhotoNum];
            }
        }
        else if (model.type == WALMEPhotoModelMediaTypeVideo) {
//                || model.type == WALMEPhotoModelMediaTypeCameraVideo
//            if (manager.maxPhotoNum > 0) {
////                if (!manager.selectTogether) { // 是否支持图片视频同时选择
...
////                }
//            }
            if (manager.selectedVideos.count == manager.maxVideoNum) {
                // 已经达到视频最大选择数
                return [NSString stringWithFormat:[NSBundle hx_localizedStringForKey:@"最多只能选择%ld个视频"],manager.maxVideoNum];
            }
        }
    }
    else if (manager.selectionType == WALMEAlbumListSelectionTypePhoto) {
        if (manager.selectedPhotos.count == manager.maxPhotoNum) {
            // 已经达到图片最大选择数
            return [NSString stringWithFormat:[NSBundle hx_localizedStringForKey:@"最多只能选择%ld张图片"],manager.maxPhotoNum];
        }
    }
    else if (manager.selectionType == WALMEAlbumListSelectionTypeVideo) {
        if (manager.selectedVideos.count == manager.maxVideoNum) {
            // 已经达到视频最大选择数
            return [NSString stringWithFormat:[NSBundle hx_localizedStringForKey:@"最多只能选择%ld个视频"],manager.maxVideoNum];
        }
    ...
    if (model.type == WALMEPhotoModelMediaTypeVideo) {
        if (model.asset.duration < manager.minVideoDuration) {
            NSString * note = [NSString stringWithFormat:@"视频少于%.0f秒,无法选择", (float)manager.minVideoDuration];
            return [NSBundle hx_localizedStringForKey:note];
        } else if (floor(model.asset.duration) > manager.maxVideoDuration) {
            NSString * note = [NSString stringWithFormat:@"视频大于%.0f秒,无法选择", (float)manager.maxVideoDuration];
            return [NSBundle hx_localizedStringForKey:note];
        }
    }
    return nil;
...
//            else
            ...
            }/*else if (model.type == WALMEPhotoModelMediaTypeLive) {
...
+ (void)getSelectedListResultModel:(NSArray<HXPhotoModel *> *)selectedList complete:(void (^)(NSArray<HXPhotoResultModel *> *, NSArray<HXPhotoResultModel *> *, NSArray<HXPhotoResultModel *> *))complete {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        for (HXPhotoModel *photoModel in selectedList) {
            ...
            if (photoModel.asset) {
                if (photoModel.subType == WALMEPhotoModelMediaSubTypeVideo) {
...
//                        }
                ...
                if (photoModel.subType == WALMEPhotoModelMediaSubTypePhoto) {
                    resultModel.photoIndex = photoModel.endIndex;
                    resultModel.type = HXPhotoResultModelMediaTypePhoto;
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        for (HXPhotoModel *photoModel in selectList) {
            if (photoModel.subType == WALMEPhotoModelMediaSubTypePhoto) {
                NSString *suffix;
                if (photoModel.asset) {
                    if (photoModel.type == WALMEPhotoModelMediaTypeGif) {
                        suffix = @"gif";
...
+ (AVAssetExportSession *)compressedVideoWithMediumQualityWriteToTemp:(HXPhotoModel *)model progress:(void (^)(float progress))progress success:(void (^)(void))success failure:(void (^)(void))failure {
    AVAsset *avAsset;
    if (model.avAsset) {
