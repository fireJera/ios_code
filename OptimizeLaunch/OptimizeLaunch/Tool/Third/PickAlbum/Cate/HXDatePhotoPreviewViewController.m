...
//SELF_WALME_CODE_ADDDATERESPONDER
//    if (!model) {
//        return nil;
//    }
...
//            }
//        }else if (model.type == WALMEPhotoModelMediaTypeCameraPhoto || model.type == WALMEPhotoModelMediaTypeCameraVideo) {
...
#pragma mark - HXDatePhotoPreviewViewCell
...
//        if (model.type == WALMEPhotoModelMediaTypeCameraPhoto || model.type == WALMEPhotoModelMediaTypeCameraVideo) {
...
//            }
    ...
    if (model.subType == WALMEPhotoModelMediaSubTypeVideo) {
...
- (void)requestHDImage {
    ...
    if (self.model.type == WALMEPhotoModelMediaTypeLive) {
    ...
    }else if (self.model.type == WALMEPhotoModelMediaTypePhoto) {
    ...
    }else if (self.model.type == WALMEPhotoModelMediaTypeGif) {
        if (self.gifImage) {
    ...
    if (self.model.type == WALMEPhotoModelMediaTypeVideo) {
        self.requestID = [HXPhotoTools getPlayerItemWithPHAsset:self.model.asset startRequestIcloud:^(PHImageRequestID cloudRequestId) {
            ...
            [[NSNotificationCenter defaultCenter] addObserver:weakSelf selector:@selector(p_walme_pausePlayerAndShowNaviBar) name:AVPlayerItemDidPlayToEndTimeNotification object:weakSelf.player.currentItem];
        } failed:^(NSDictionary *info) {
            weakSelf.videoPlayBtn.userInteractionEnabled = YES;
            weakSelf.progressView.hidden = YES;
        }];
    }
//    else if (self.model.type == WALMEPhotoModelMediaTypeCameraVideo ) {
//        self.player = [AVPlayer playerWithPlayerItem:[AVPlayerItem playerItemWithURL:self.model.videoURL]];
//        self.playerLayer.player = self.player;
//        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(p_walme_pausePlayerAndShowNaviBar) name:AVPlayerItemDidPlayToEndTimeNotification object:self.player.currentItem];
//    }
}
- (void)p_walme_pausePlayerAndShowNaviBar {
    [self.player pause];
...
- (void)cancelRequest {
    ...
    if (self.model.subType == WALMEPhotoModelMediaSubTypeVideo) {
        if (self.player != nil) {
...
- (UIScrollView *)scrollView {
    if (!_scrollView) {
//        SELF_WALME_CODE_ADDDATEIMAGEVIEW
