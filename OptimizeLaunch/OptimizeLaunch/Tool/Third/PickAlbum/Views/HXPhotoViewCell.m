...
//        [self.selectBtn setImage:[UIImage imageNamed:@"checkbox_def"] forState:UIControlStateNormal];
//        [self.selectBtn setImage:[UIImage imageNamed:@"checkbox_act"] forState:UIControlStateSelected];
    }
...
- (void)setModel:(HXPhotoModel *)model {
    ...
    } else if (model.type == WALMEPhotoModelMediaTypePhoto || (model.type == WALMEPhotoModelMediaTypeGif || model.type == WALMEPhotoModelMediaTypeLive)){
    ...
    } else if (model.type == WALMEPhotoModelMediaTypeCamera){
...
- (void)didSelectClick:(UIButton *)button {
    if (self.model.type == WALMEPhotoModelMediaTypeCamera) {
        return;
    }
...
- (UIButton *)selectBtn {
    if (!_selectBtn) {
        ...
        [_selectBtn setBackgroundImage:[UIImage imageNamed:@"checkbox_act"] forState:UIControlStateSelected];
        [_selectBtn addTarget:self action:@selector(didSelectClick:) forControlEvents:UIControlEventTouchUpInside];
        _selectBtn.titleLabel.font = [UIFont walme_PingFangMedium14];
        _selectBtn.frame = CGRectMake(self.hx_w - 31, self.hx_w - 38, 30, 30);
