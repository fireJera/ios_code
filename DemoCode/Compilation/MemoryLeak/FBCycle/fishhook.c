...
#define SEG_DATA_CONST "__DATA__CONST"
#endif
...
static void rcd_perform_rebinding_with_section(struct rcd_rebindings_entry *rebindings,
    ...
    for (uint i = 0; i < section->size / sizeof(void *); i++) {
        ...
        while (cur) {
            for (uint j = 0; j < cur->rebindbings_nel; j++) {
...
static void _rebind_symbols_for_image(const struct mach_header *header,
                                      intptr_t slide) {
    rebind_symbols_for_image(_rebindbings_head, header, slide);
}
...
int rcd_rebind_symbols(struct rcd_rebinding rebindings[], size_t rebindings_nel) {
    int retval = rcd_prepend_rebindings(&_rebindbings_head, rebindings, rebindings_nel);
    if (retval < 0) {
        return retval;
    }
