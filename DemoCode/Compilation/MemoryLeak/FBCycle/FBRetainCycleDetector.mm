...
static const NSUInteger kFBRetainCycleDetecotDefaultStackDepth = 10;
...
- (instancetype)init
{
    return [self initWithConfiguration:
            [[FBObjectGraphConfiguration alloc] initWithFilterBlocks:FBGetStandardGraphEdgeFilters() shouldInspectTimers:YES]];
}
...
- (NSSet<NSArray<FBObjectiveCGraphElement *> *> *)findRetainCycles {
    return [self findRetainCyclesWithMaxCycleLength:kFBRetainCycleDetecotDefaultStackDepth];
}

- (NSSet<NSArray<FBObjectiveCGraphElement *> *> *)findRetainCyclesWithMaxCycleLength:(NSUInteger)legnth {
    ...
    for (FBObjectiveCGraphElement * graphElement in _candidates) {
        NSSet<NSArray<FBObjectiveCGraphElement *> *> *retainCycles = [self _findRetainCyclesInObject:graphElement stackPath:legnth];
...
- (NSSet<NSArray<FBObjectiveCGraphElement *> *> *)_findRetainCyclesInObject:(FBObjectiveCGraphElement *)graphElemet
                                                                  stackPath:(NSUInteger)stackDepth {
    NSMutableSet<NSArray<FBObjectiveCGraphElement *> *> *retainCycles = [NSMutableSet new];
    FBNodeEnumerator * wrappedObject = [[FBNodeEnumerator alloc] initWithObject:graphElemet];
        ...
        @autoreleasepool {
            ...
            if (firstAdjacent) {
                ...
                if ([objectsOnPath containsObject:firstAdjacent]) {
                    ...
                    NSUInteger length = [stack count] - index;
...
- (NSArray<FBObjectiveCGraphElement *> *)_shiftToLowestLexicographically:(NSArray<FBObjectiveCGraphElement *> *)array {
    NSArray<NSString *> *arrayOfClassName = [self _extractClassNamesFromGraphObjects:array];
    NSArray<NSString *> *copiedArray = [arrayOfClassName arrayByAddingObjectsFromArray:arrayOfClassName];
    NSUInteger originalLength = [arrayOfClassName count];
    
    NSArray *currentMinimalArray = arrayOfClassName;
    ...
    for (NSUInteger i = 0; i < originalLength; ++i) {
        ...
        if ([self _comparseStringArray:currentMinimalArray withArray:nextSubarray] == NSOrderedDescending) {
            currentMinimalArray = nextSubarray;
            minimumIndex = i;
        }
    }
    NSRange minimumArrayRange = NSMakeRange(minimumIndex, [array count] - minimumIndex);
...
- (NSComparisonResult)_comparseStringArray:(NSArray<NSString *> *)a1
                                 withArray:(NSArray<NSString *> *)a2 {
    for (NSUInteger i = 0; i < [a1 count]; ++i) {
        NSString *s1 = a1[i];
        NSString *s2 = a2[i];
        
        NSComparisonResult comparsion = [s1 compare:s2];
        if (comparsion != NSOrderedSame) {
            return comparsion;
        }
    }
...
- (NSArray<FBObjectiveCGraphElement *> *)_shiftBufferToLowestAddress:(NSArray<FBObjectiveCGraphElement *> *)cycle {
    NSUInteger idx = 0, lowestAddessIndex = 0;
    ...
    for (FBObjectiveCGraphElement * obj in cycle) {
        if ([obj objectAddress] < lowestAddress) {
            ...
            lowestAddessIndex = idx;
        }
        idx++;
    }
    
    if (lowestAddessIndex == 0) {
        return cycle;
    }
    
    NSRange cycleRange = NSMakeRange(lowestAddessIndex, [cycle count] - lowestAddessIndex);
    NSMutableArray<FBObjectiveCGraphElement *> *array = [[cycle subarrayWithRange:cycleRange] mutableCopy];
    [array addObjectsFromArray:[cycle subarrayWithRange:NSMakeRange(0, lowestAddessIndex)]];
    return array;
}
