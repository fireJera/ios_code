...
//        [_selectBtn setTitleColor:[UIColor blackTextColor] forState:UIControlStateNormal];
        [_selectBtn setBackgroundImage:[UIImage imageNamed:@"letad_album_2"] forState:UIControlStateNormal];
        [_selectBtn addTarget:self action:@selector(didSelectClick:) forControlEvents:UIControlEventTouchUpInside];
        _selectBtn.frame = CGRectMake(self.hx_w - 31, 0, 30, 30);
...
//        [self.selectBtn setImage:[UIImage imageNamed:@"letad_album_2"] forState:UIControlStateNormal];
//        [self.selectBtn setImage:[UIImage imageNamed:@"letad_album_1"] forState:UIControlStateSelected];
...
- (void)didSelectClick:(UIButton *)button {
    ...
    //视频不让多选
//    if (self.model.type == HXPhotoModelMediaTypePhotoVideo) {
//        return;
//    }
//    if (self.model.type == HXPhotoModelMediaTypePhotoCameraVideo) {
//        return;
//    }
...
- (void)setModel:(HXPhotoModel *)model {
    ...
    if (model.type == HXPhotoModelMediaTypePhotoVideo) {
        ...
        if (model.videoSecond > _maxTime || model.videoSecond < _minTime) {
            ...
            _bottomView.frame = CGRectMake(0, self.hx_h - 25, self.hx_w, 25);
            _bottomView.backgroundColor = [[UIColor blackColor] colorWithAlphaComponent:0.6];
        }
        _videoTime.frame = CGRectMake(0, self.bottomView.height - 25, self.bottomView.width - 5, 25);
    } else if (model.type == HXPhotoModelMediaTypePhoto || (model.type == HXPhotoModelMediaTypePhotoGif || model.type == HXPhotoModelMediaTypePhotoLivePhoto)){
    ...
    if (model.selected) {
        [self.selectBtn setBackgroundImage:[UIImage imageNamed:@"letad_album_1"] forState:UIControlStateSelected];
        ...
        [self.selectBtn setBackgroundImage:[UIImage imageNamed:@"letad_album_2"] forState:UIControlStateNormal];
    }
}
- (void)cancelRequest {
    if (self.requestID) {
