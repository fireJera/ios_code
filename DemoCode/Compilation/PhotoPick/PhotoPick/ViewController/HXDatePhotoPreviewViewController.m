...
- (void)requestHDImage {
    ...
    if (self.model.type == HXPhotoModelMediaTypePhotoLivePhoto) {
    ...
    if (self.model.type == HXPhotoModelMediaTypePhotoVideo) {
        self.requestID = [HXPhotoTools getPlayerItemWithPHAsset:self.model.asset startRequestIcloud:^(PHImageRequestID cloudRequestId) {
            ...
            [[NSNotificationCenter defaultCenter] addObserver:weakSelf selector:@selector(p_letad_pausePlayerAndShowNaviBar) name:AVPlayerItemDidPlayToEndTimeNotification object:weakSelf.player.currentItem];
        } failed:^(NSDictionary *info) {
            weakSelf.videoPlayBtn.userInteractionEnabled = YES;
            weakSelf.progressView.hidden = YES;
        }];
    }else if (self.model.type == HXPhotoModelMediaTypePhotoCameraVideo ) {
        self.player = [AVPlayer playerWithPlayerItem:[AVPlayerItem playerItemWithURL:self.model.videoURL]];
        self.playerLayer.player = self.player;
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(p_letad_pausePlayerAndShowNaviBar) name:AVPlayerItemDidPlayToEndTimeNotification object:self.player.currentItem];
    }
}
