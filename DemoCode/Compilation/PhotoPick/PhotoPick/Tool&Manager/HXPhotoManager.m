...
- (void)getPhotoListWithAlbumModel:(HXAlbumModel *)albumModel complete:(void (^)(NSArray *allList , NSArray *previewList,NSArray *photoList ,NSArray *videoList ,NSArray *dateList , HXPhotoModel *firstSelectModel))complete {
        ...
        [albumModel.result enumerateObjectsWithOptions:NSEnumerationReverse usingBlock:^(PHAsset *asset, NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            if (selectList.count > 0) {
                ...
                if (newArray.count > 0) {
                    ...
                    photoModel.selected = YES;
                    if ((model.type == HXPhotoModelMediaTypePhoto || model.type == HXPhotoModelMediaTypePhotoGif) || (model.type == HXPhotoModelMediaTypePhotoLivePhoto || model.type == HXPhotoModelMediaTypePhotoCameraPhoto)) {
                        ...
                        if (model.type == HXPhotoModelMediaTypePhotoCameraVideo) {
            ...
            if (asset.mediaType == PHAssetMediaTypeImage) {
                ...
                if ([[asset valueForKey:@"filename"] hasSuffix:@"GIF"]) {
                    ...
                    if (self.hasLivePhoto) {
                        if (@available(iOS 9.1, *)) {
                            if (asset.mediaSubtypes == PHAssetMediaSubtypePhotoLive) {
                                if (!self.singleSelected) {
                                    photoModel.type = self.lookLivePhoto ? HXPhotoModelMediaTypePhotoLivePhoto : HXPhotoModelMediaTypePhoto;
                                }else {
                                    photoModel.type = HXPhotoModelMediaTypePhoto;
                                }
                            }else {
    ...
    if (self.openCamera) {
        ...
        model.type = HXPhotoModelMediaTypePhotoCamera;
