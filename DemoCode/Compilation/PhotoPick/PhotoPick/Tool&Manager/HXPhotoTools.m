...
+ (NSString *)maximumOfJudgment:(HXPhotoModel *)model manager:(HXPhotoManager *)manager {
    if (manager.selectedList.count == manager.maxNum) {
        ...
        return [NSString stringWithFormat:@"最多只能选择%ld个" ,manager.maxNum];
    }
    if (manager.type == HXPhotoManagerSelectedTypePhotoAndVideo) {
        if ((model.type == HXPhotoModelMediaTypePhoto || model.type == HXPhotoModelMediaTypePhotoGif) || (model.type == HXPhotoModelMediaTypePhotoCameraPhoto || model.type == HXPhotoModelMediaTypePhotoLivePhoto)) {
            ...
            if (manager.selectedPhotos.count == manager.photoMaxNum) {
                ...
                return [NSString stringWithFormat:@"最多只能选择%ld张图片",manager.photoMaxNum];
            }
            ...
            if (manager.selectedVideos.count == manager.videoMaxNum) {
                ...
                return [NSString stringWithFormat:@"最多只能选择%ld个视频",manager.videoMaxNum];
            }
        }
    ...
    if (model.type == HXPhotoModelMediaTypePhotoVideo) {
        if (model.asset.duration < manager.videoMinDuration) {
            NSString * note = [NSString stringWithFormat:@"视频少于%.0f秒,无法选择", manager.videoMinDuration];
            return note;
        } else if (floor(model.asset.duration) > manager.videoMaxDuration) {
            NSString * note = [NSString stringWithFormat:@"视频大于%.0f秒,无法选择", manager.videoMaxDuration];
            return note;
        }
    }
    return nil;
