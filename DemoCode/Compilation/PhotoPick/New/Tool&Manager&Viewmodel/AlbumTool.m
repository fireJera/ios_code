...
//                                                   size:(CGSize)size
//                                             completion:(void(^)(UIImage *image,NSDictionary *info))completion error:(void(^)(NSDictionary *info))error {
//    PHImageRequestOptions *option = [[PHImageRequestOptions alloc] init];
...
//                                    size:(CGSize)size
//                            deliveryMode:(PHImageRequestOptionsDeliveryMode)deliveryMode completion:(void (^)(UIImage *, NSDictionary *))completion  {
//    PHImageRequestOptions *option = [[PHImageRequestOptions alloc] init];
...
//}
//
+ (NSString *)maximumOfJudgment:(PhotoModel *)model viewmodel:(AlbumListViewmodel *)viewmodel {
    if (viewmodel.selectedList.count == viewmodel.maxNum) {
        return [NSString stringWithFormat:@"最多只能选择%ld个", viewmodel.maxNum];
    }
    if (viewmodel.selectionType == AlbumListSelectionTypeAll) {
...
//             model.type == PhotoModelMediaTypeLive)) {
            if (viewmodel.selectedPhotos.count == viewmodel.maxPhotoNum) {
                // 已经达到图片最大选择数
                return [NSString stringWithFormat:@"最多只能选择%ld张图片", viewmodel.maxPhotoNum];
            }
        }
        else if (model.type == PhotoModelMediaTypeVideo) {
//            else if (model.type == PhotoModelMediaTypeVideo || model.type == PhotoModelMediaTypeCameraVideo) {
            if (viewmodel.maxPhotoNum > 0) {
                if (viewmodel.selectedPhotos.count > 0 ) {
                    // 已经选择了图片,不能再选视频
                    return @"视频不能和图片同时选择";
                }
            }
            if (viewmodel.selectedVideos.count == viewmodel.maxVideoNum) {
                // 已经达到视频最大选择数
                return [NSString stringWithFormat:@"最多只能选择%ld个视频", viewmodel.maxVideoNum];
            }
        }
    }
    else if (viewmodel.selectionType == AlbumListSelectionTypePhoto) {
        if (viewmodel.selectedPhotos.count == viewmodel.maxPhotoNum) {
            // 已经达到图片最大选择数
            return [NSString stringWithFormat:@"最多只能选择%ld张图片", viewmodel.maxPhotoNum];
        }
    }else if (viewmodel.selectionType == AlbumListSelectionTypeVideo) {
        if (viewmodel.selectedVideos.count == viewmodel.maxVideoNum) {
            // 已经达到视频最大选择数
            return [NSString stringWithFormat:@"最多只能选择%ld个视频",viewmodel.maxVideoNum];
        }
    ...
    if (model.type == PhotoModelMediaTypeVideo) {
        if (model.asset.duration < viewmodel.minVideoDuration) {
            return [NSString stringWithFormat:@"视频少于%ld秒,无法选择", viewmodel.minVideoDuration];
        }else if (model.asset.duration > viewmodel.maxVideoDuration) {
            return [NSString stringWithFormat:@"视频大于%ld,无法选择", viewmodel.minVideoDuration];
        }
    }
    return nil;
}
...
//+ (void)getImageForSelectedPhoto:(NSArray<PhotoModel *> *)photos type:(HXPhotoToolsFetchType)type completion:(void (^)(NSArray<UIImage *> *))completion {
//    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
...
//                    });
//                }];
//            } else if (model.type == PhotoModelMediaTypePhotoGif) {
...
//                    }];
//                }];
//            }/*else if (model.type == PhotoModelMediaTypeLivePhoto) {
...
//+ (void)sortImageForModel:(PhotoModel *)model total:(NSInteger)total images:(NSMutableArray *)images completion:(void(^)(NSArray *array))completion {
//    [images addObject:model];
//    if (images.count == total) {
//        [images sortUsingComparator:^NSComparisonResult(PhotoModel *temp, PhotoModel *other) {
...
//}
//
//+ (void)getSelectedPhotosFullSizeImageUrl:(NSArray<PhotoModel *> *)photos complete:(void (^)(NSArray<NSURL *> *))complete {
//    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
...
//}
//
//+ (void)sortFullImageUrlForModel:(PhotoModel *)model total:(NSInteger)total images:(NSMutableArray *)images completion:(void(^)(NSArray *array))completion {
//    [images addObject:model];
//    if (images.count == total) {
//        [images sortUsingComparator:^NSComparisonResult(PhotoModel *temp, PhotoModel *other) {
//            NSInteger length1 = temp.endIndex;
...
//}
//
//+ (void)sortResultModel:(PhotoResultModel *)model
//                  total:(NSInteger)total
//                 models:(NSMutableArray *)models
//                 photos:(NSMutableArray *)photos
//                 videos:(NSMutableArray *)videos
//             completion:(void(^)(NSArray *all, NSArray *photos, NSArray *videos))completion {
//    [models addObject:model];
//    if (model.mediaType == PhotoResultModelMediaTypePhoto) {
//        [photos addObject:model];
//    }else {
//        [videos addObject:model];
//    }
//    if (models.count == total) {
//        [models sortUsingComparator:^NSComparisonResult(PhotoResultModel *temp, PhotoResultModel *other) {
...
//            return result == NSOrderedDescending;
//        }];
//        [photos sortUsingComparator:^NSComparisonResult(PhotoResultModel *temp, PhotoResultModel *other) {
...
//            return result == NSOrderedDescending;
//        }];
//        [videos sortUsingComparator:^NSComparisonResult(PhotoResultModel *temp, PhotoResultModel *other) {
...
//+ (void)getSelectedListResultModel:(NSArray<PhotoModel *> *)selectedList complete:(void (^)(NSArray<PhotoResultModel *> *, NSArray<PhotoResultModel *> *, NSArray<PhotoResultModel *> *))complete {
//    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
//        NSMutableArray *models = [NSMutableArray array];
//        NSMutableArray *photos = [NSMutableArray array];
//        NSMutableArray *videos = [NSMutableArray array];
//        for (PhotoModel *photoModel in selectedList) {
//            PhotoResultModel *resultModel = [[PhotoResultModel alloc] init];
...
//                    }
//                    resultModel.type = PhotoResultModelMediaTypeVideo;
//                    resultModel.videoIndex = photoModel.videoIndex;
//                }else {
//                    resultModel.photoIndex = photoModel.endIndex;
//                    resultModel.type = PhotoResultModelMediaTypePhoto;
//                }
...
//                        if (resultModel.type == PhotoResultModelMediaTypePhoto) {
...
//        for (PhotoModel *photoModel in selectList) {
//            if (photoModel.subType == HXPhotoModelMediaSubTypePhoto) {
//                NSString *suffix;
//                if (photoModel.asset) {
//                    if (photoModel.type == PhotoModelMediaTypePhotoGif) {
//                        suffix = @"gif";
...
//+ (void)writeOriginalImageToTempWith:(PhotoModel *)model
//                           requestId:(void(^)(PHImageRequestID requestId))requestId
//                     iCloudRequestId:(void(^)(PHImageRequestID requestId))iCloudRequestId
...
//}
//
//+ (AVAssetExportSession *)compressedVideoWithMediumQualityWriteToTemp:(PhotoModel *)model
...
+ (PHImageRequestID)getImageData:(PHAsset *)asset
              startRequestIcloud:(void(^)(PHImageRequestID cloudRequestId))startRequestIcloud
                 progressHandler:(void(^)(double progress))progressHandler
                      completion:(void(^)(NSData *imageData, UIImageOrientation orientation))completion failed:(void(^)(NSDictionary *info))failed {
