...
static inline dispatch_queue_priority_t NSQualityOfServiceToDispatchPriority(NSQualityOfService qos){
    switch (qos) {
        ...
        default: return NSQualityOfServiceDefault;
    }
}
...
static YYDispatchContext * YYDispatchContextCreate(const char *name,
    ...
    if ([UIDevice currentDevice].systemVersion.floatValue >= 8.0) {
        ...
        for (NSUInteger i = 0; i < queueCount; i++) {
            ...
            context->queues[i] = (__bridge void *)queue;
        }
    } else {
...
static YYDispatchContext * YYDispatchContextGetForQOS(NSQualityOfService qos) {
    static YYDispatchContext * contexts[5] = {0};
    switch (qos) {
        case NSQualityOfServiceUserInteractive: {
            ...
            dispatch_once(&onceToken, ^{
                ...
                contexts[0] = YYDispatchContextCreate("com.dispatchqueuePool.user-interactive", count, qos);
            });
            return contexts[0];
        }
        case NSQualityOfServiceUserInitiated: {
            ...
            dispatch_once(&onceToken, ^{
                int count = (int)[NSProcessInfo processInfo].activeProcessorCount;
                count = count < 1 ? 1 : count > MAX_QUEUE_COUNT ? MAX_QUEUE_COUNT : count;
                contexts[1] = YYDispatchContextCreate("com.dispatchqueuePool.user-initiated", count, qos);
            });
            return contexts[1];
        }
        case NSQualityOfServiceUtility: {
            ...
            dispatch_once(&onceToken, ^{
                int count = (int)[NSProcessInfo processInfo].activeProcessorCount;
                count = count < 1 ? 1 : count > MAX_QUEUE_COUNT ? MAX_QUEUE_COUNT : count;
                contexts[2] = YYDispatchContextCreate("com.dispatchqueuePool.Utility", count, qos);
            });
            return contexts[2];
        }
        case NSQualityOfServiceBackground: {
            ...
            dispatch_once(&onceToken, ^{
                int count = (int)[NSProcessInfo processInfo].activeProcessorCount;
                count = count < 1 ? 1 : count > MAX_QUEUE_COUNT ? MAX_QUEUE_COUNT : count;
                contexts[3] = YYDispatchContextCreate("com.dispatchqueuePool.Background", count, qos);
            });
            return contexts[4];
        }
        ...
        default: {
            ...
            dispatch_once(&onceToken, ^{
                int count = (int)[NSProcessInfo processInfo].activeProcessorCount;
                count = count < 1 ? 1 : count > MAX_QUEUE_COUNT ? MAX_QUEUE_COUNT : count;
                contexts[0] = YYDispatchContextCreate("com.dispatchqueuePool.Default", count, qos);
            });
            return contexts[0];
        }
    }
